...
public class WhiteTestService {
    ...
    public WhiteTestResponse checkWhiteTest(StudentRequest studentRequest){
        ...
        if(!test_student)
            whiteTestResponse.getCriteriaMismatch().add("cet élève n'existe pas");

        List<Exam> exams = dataTest.listOfExam;
        boolean test_exam = exams.stream().anyMatch(x->x.getCode().equals(exam_code));
        if(!test_exam)
            whiteTestResponse.getCriteriaMismatch().add("cet examen n'existe pas");
        ...
        if(whiteTestResponse.getCriteriaMismatch().isEmpty()){
            ...
            whiteTestResponse.getCriteriaMismatch().add("le code d'examen n'est pas joignable pour l'étudiant actuel");
        }
